...
def eq81(CB, Re, Rm, kmax, lambda_max):
    K2 = 1.0 + kmax**2.0
    kz2 = kmax**2.0
...
ks = np.linspace(ks[k_max_ind-2], ks[k_max_ind+2], num=100)
gams = kolmogorov_EVP.gamma_over_k(0.0, CB, Re, Rm, ks, N)
k_max_ind = np.argmax(gams)
kmax = ks[k_max_ind]
...
print(eq81(CB, Re, Rm, kmax, lambda_max))
...
print('A_1/psi_0: ')
print((psi[int(N/2) + 1]/psi[int(N/2)])*(psi[int(N/2)]*-1.0j/phi[int(N/2)]))
print((eq83(CB, Re, Rm, kmax, lambda_max))*(eq82(CB, Re, Rm, kmax, lambda_max)))

print('Comparing the approximate expressions, which follow in the appendix: ')
print('psi_1/psi_0: ')
print(phi[int(N/2)+1]/phi[int(N/2)])
print(eq85(CB, Re, Rm, kmax)[1])
...
print('A_1/A_0: ')
print(psi[int(N/2) + 1]/psi[int(N/2)])
print(eq87(CB, Re, Rm, kmax)[1])

print('A_1/psi_0: ')
print((psi[int(N/2) + 1]/psi[int(N/2)])*(psi[int(N/2)]*-1.0j/phi[int(N/2)]))
print((eq87(CB, Re, Rm, kmax)[1])*(eq86(CB)[1]))
